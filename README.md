...
**상점 1차 오픈 — 무료 코스메틱 10종**
- `items` 테이블에 `name`/`price` 컬럼 추가, `slot_type`에 10종(모자/얼굴/상의/바지/신발/방패/장신구/안경/이펙트/배경) 추가, `owner_type`에 `shop`(누구 소유도 아닌 판매용 카탈로그) 추가 — 전부 테이블 재생성 방식(008/014/017/020과 동일 패턴)
- 시드 10개(월계관/철학자 수염/키톤/가죽 각반/가죽 샌들/호플론 방패/올리브 브로치/둥근테 안경/빛무리/기둥 배경) — 전부 가격 0원, `owner_type='shop'`이라 종교/정당/지배자 소속 같은 착용 조건 검증 대상이 아님(누구나 획득 가능)
- API: `GET /items/shop`(카탈로그), `POST /items/:id/acquire`(무료 아이템 전용, 결제 없이 바로 인벤토리 추가, 중복 획득은 409)
- 프론트: 상점 탭에 그리드로 표시, 아이콘은 전부 인라인 SVG로 직접 그림(이 프로젝트는 이미지 업로드/호스팅이 없어서 기존 아바타·지도 아이콘과 동일한 방식) — 로그인 안 하면 "로그인 필요", 보유 중이면 "보유중"(비활성화) 표시
